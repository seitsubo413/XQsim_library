...
OUTPUT_DIR = "/app/test_results_2q_multi_stage"

# 2量子ビット多段回路のテストケース
TEST_CASES = [
    {
        "name": "test_2q_double_cnot_chain",
        "description": "2量子ビット: 複数CNOT連鎖（H→CNOT→H→CNOT）",
        "qasm": """OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
measure q[1] -> c[1];"""
    ...
    {
        "name": "test_2q_triple_cnot_chain",
        "description": "2量子ビット: 3段CNOT連鎖（H→CNOT→X→CNOT→Z→CNOT）",
        "qasm": """OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
measure q[1] -> c[1];"""
    ...
    {
        "name": "test_2q_alternating_gates",
        "description": "2量子ビット: 交互ゲート（H→X→CNOT→H→X→CNOT）",
        "qasm": """OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
measure q[1] -> c[1];"""
    ...
    {
        "name": "test_2q_multi_h_cnot",
        "description": "2量子ビット: 複数H+CNOTパターン（5段）",
        "qasm": """OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
measure q[1] -> c[1];"""
    ...
    {
        "name": "test_2q_xz_cnot_chain",
        "description": "2量子ビット: X/Z+CNOT連鎖（4段）",
        "qasm": """OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
def main():
    ...
    print("=" * 70)
    print("XQsim API テスト - 2量子ビット多段回路")
    print("=" * 70)
    print()
